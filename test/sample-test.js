...
describe("PurchaseHook", function () {
  it("Should work", async function () {
    ...
    const [data, signerAddress] = await getSignatureForPassword(password, recipient)

    // with wrong password
    const [badData, _] = await getSignatureForPassword('wrongpassword', recipient)
    expect(await hook.getSigner(recipient.toLowerCase(), badData)).to.not.equal(signerAddress);

    // with correct password
    expect(await hook.getSigner(recipient.toLowerCase(), data)).to.equal(signerAddress);
  });
  ...
  it('should work as a hook', async function () {
    ...
    const [data, signer] = await getSignatureForPassword(password, user.address.toLowerCase())
    ...
    await expect(lock.purchase([0], [user.address], [user.address], [user.address], [])).to.reverted

    // And a purchase that fails because we use the wrong password
    const [badData,] = await getSignatureForPassword("wrong password", user.address.toLowerCase())
    await expect(lock.purchase([0], [user.address], [user.address], [user.address], [badData])).to.reverted

    // And a purchase that succeeds when we use the correct password!
    await expect(lock.purchase([0], [user.address], [user.address], [user.address], [data])).not.to.reverted
